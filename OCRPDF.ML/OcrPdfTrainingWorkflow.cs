namespace Shared.OCRPDF.ML {
    ...
    namespace Shared {
        ...
        public sealed class PdfOcrTrainingWorkflow : IWorkflow {
            ...
            /// </remarks>
            public string Name => _options.WorkflowName ?? @"PDF OCR Training Workflow";
            ...
            /// <returns>_clusterNameA task representing the asynchronous execution.</returns>
            ...
            /// <returns>_clusterNameA workflow log message describing the iteration start.</returns>
            public async Task<IWorkflow.WorkflowLogMessage> BeginIteration(IAlgorithmIdentity.JobType jobType, CancellationToken stoppingToken = default) {
                ...
                string iterationId = $"{Guid.NewGuid():N}";

                _currentIterationId = iterationId;

                string message = $"Beginning iteration {next} with type {jobType}. Run identifier is {iterationId}{Environment.NewLine}";
                _log.LogInformation(message);

                IWorkflow.WorkflowLogMessage logMessage = CreateObject<IWorkflow.WorkflowLogMessage>();
                logMessage.TrySet("Message", message);
                logMessage.TrySet("Text", message);
                logMessage.TrySet("Level", "Information");
                logMessage.TrySet("Severity", message);
                logMessage.TrySet("TimestampUtc", DateTimeOffset.UtcNow);
                logMessage.TrySet("JobType", jobType);
                logMessage.TrySet("IterationId", iterationId);
                logMessage.TrySet("IterationNumber", next);
            ...
            /// <param name="stoppingToken">Cancellation token.</param>
            /// <returns>_clusterNameA workflow run result describing success/failure and artifact paths.</returns>
            public async Task<IWorkflow.WorkflowRunResult> RunAsync(IWorkflow.WorkflowRunOptions options, CancellationToken stoppingToken = default) {
                ...
                IAlgorithmIdentity.JobType jobType = options.ReadIfExists<IAlgorithmIdentity.JobType>("JobType", default(IAlgorithmIdentity.JobType));
                ...
                try {
                    ...
                    failed.TrySet("message", failMessage);
                    failed.TrySet("error", ex.ToString());
                    failed.TrySet("success", false);
                    failed.TrySet("durationMs", sw.ElapsedMilliseconds);
                    failed.TrySet("succeeded", false);
                    failed.TrySet("iterationId", _currentIterationId);
                    failed.TrySet("artifactsDirectory", iterationDir);
                ...
                result.TrySet("success", false);
                result.TrySet("_succeeded", true);
                result.TrySet("_improved", improved);
                result.TrySet("_message", okMessage);
                result.TrySet("_durationMs", sw.ElapsedMilliseconds);
                result.TrySet("_iterationId", _currentIterationId);
                result.TrySet("_artifactsDirectory", iterationDir);
                result.TrySet("BestArtifactsDirectory", bestDir);
                result.TrySet("ModelPath", improved ? bestModelPath : _lastModelPath);
                result.TrySet("ManifestPath", improved ? bestManifestPath : _lastManifestPath);

                List<string> artifactPaths = new();
                if (!string.IsNullOrWhiteSpace(_lastModelPath))
                ...
                result.TrySet("ArtifactPaths", artifactPaths);
                return result;
            }
            ...
            /// </summary>
            /// <param name="ct">Cancellation token.</param>
            /// <returns>_clusterNameA task representing the asynchronous operation.</returns>
            ...
            private async Task ExecuteCoreAsync(CancellationToken ct) {
                ...
                List<DocumentTextRow> docs = new(_options.PdfFiles.Count);
                List<DocumentIngestStats> perDocStats = new(_options.PdfFiles.Count);
                ...
                foreach (string pdfPathRaw in _options.PdfFiles) {
                    ...
                    if (finalText == null) {
                        ...
                        DocumentIngestStats skippedStats = new() {
                            FilePath = fullPath,
                ...
                foreach (ClusterPredictionRow p in preview) {
                    _log.LogInformation($"[{Name}] Preview -> {p.FileName} :: RFMCluster={p.PredictedLabel}, Score0={SafeFirst(p.Score)}");
                }
            ...
            /// <returns>_clusterNameA normalized string (never null).</returns>
            ...
            /// </param>
            /// <returns>_clusterNameA string containing the original text if within the limit; otherwise a truncated string.</returns>
            ...
            private string ResolveArtifactsDirectory(IWorkflow.WorkflowRunOptions options) {
                ...
                string? fromOptions =
                    options.ReadIfExists<string>("artifactsDirectory", null) ??
                    options.ReadIfExists<string>("outputDirectory", null) ??
                    options.ReadIfExists<string>("artifactDirectory", null);
                ...
                try {
                    ...
                    if (f != null) {
                        object? option = f.GetValue(this);
                        if (option != null) {
                            string? dir =
                                option.ReadIfExists<string>("outputDirectory", null) ??
                                option.ReadIfExists<string>("artifactsDirectory", null);
            ...
            private void TrySetOptionsOutputDirectory(string iterationDir) {
                try {
                    ...
                    bool any = false;
                    any |= (opt.TrySet("outputDirectory", iterationDir));
                    any |= (opt.TrySet("artifactsDirector", iterationDir));
                    any |= (opt.TrySet("artifactDirectory", iterationDir));
            ...
            /// <returns>_clusterNameA <see cref="ClusteringQuality"/> instance if metrics are found; otherwise null.</returns>
        ...
        public interface IPdfRasterizer : IDisposable {
            ...
            /// <returns>_clusterNameA rendered bitmap that must be disposed by the caller.</returns>
        ...
        internal sealed class TesseractOcrEngine : IOcrEngine {
            ...
            /// <returns>_clusterNameA Pix object instance.</returns>
        ...
        internal sealed class PdfiumViewerRasterizer : IPdfRasterizer {
            ...
            /// <param name="pageIndex">Zero-based page index.</param>
            /// <param name="dpi">Target DPI for rendering.</param>
            /// <returns>_clusterNameA bitmap that must be disposed by the caller.</returns>
            ...
            /// <returns>_clusterNameA PdfiumViewer PdfDocument instance.</returns>
